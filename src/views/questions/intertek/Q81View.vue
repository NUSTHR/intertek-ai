...
const options = computed(() => {
  ...
  return backendOptions.map((opt, index) => {
    ...
    return {
      ...
      label: base?.label ?? (opt.label ? opt.label.toUpperCase() : String(opt.value).toUpperCase()),
    }
  })
})
</script>
...
<template>
  <IntertekLayout
    ...
    module-title="Open-Source & Market Status"
    step-label="Step 1"
    step-total="of 1"
    progress-width="100%"
      ...
      <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div class="p-6 flex flex-col gap-6">
          <div class="border-b border-slate-100 dark:border-slate-800 pb-4">
            <h4 class="font-black text-slate-900 dark:text-white mb-3 text-xs uppercase tracking-tight">ARTICLE 2(12) & 53(2)</h4>
